...
@Injectable()
export class LitellmClientService {
  private readonly logger = new Logger(LitellmClientService.name)
  ...
  constructor(
    ...
    this.baseUrl = this.config.get<string>('LITELLM_BASE_URL') || 'http://localhost:4000'
  ...
  private rethrowAxios(error: any): never {
    if (error?.response) {
      ...
      throw new Error(`LiteLLM ${status}: ${detail}`)
    }
    if (error?.code === 'ECONNREFUSED') {
      throw new Error(`LiteLLM not reachable at ${this.baseUrl}`)
    }
